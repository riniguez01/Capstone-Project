/**
 * Feature 3 (conversation safety) — dedicated test pair.
 *
 *   node scripts/feature3TestPair.js seed   — create/replace users + mutual match
 *   node scripts/feature3TestPair.js reset  — clear chat & safety DB state, restore match (repeat tests)
 *
 * Logins (both use password: password123):
 *   feature3a@test.com  — Alpha (Woman)
 *   feature3b@test.com  — Beta (Man)
 *
 * Loads .env from project root and backend/.env (same as other db scripts).
 */
...
const EMAIL_A = "feature3a@test.com";
const EMAIL_B = "feature3b@test.com";
...
const USERS = [
    {
        ...
        dob: "1999-06-15",
        gender: "Woman",
        bio: "Safety feature test account (Alpha). Chicago.",
        city: "Chicago",
        state: "IL",
        lat: CHI_LAT,
        lon: CHI_LON,
    ...
    },
    {
        ...
        dob: "1998-04-20",
        gender: "Man",
        bio: "Safety feature test account (Beta). Chicago.",
        city: "Chicago",
        state: "IL",
        lat: CHI_LAT,
        lon: CHI_LON,
...
async function deleteUsersAndDeps(client, emails) {
    await client.query(
        `CREATE TEMP TABLE _f3pair ON COMMIT DROP AS
         SELECT user_id FROM users WHERE email = ANY($1::text[])`,
        [emails]
    );
    const { rows } = await client.query(`SELECT user_id FROM _f3pair`);
    if (rows.length === 0) {
        console.log("No existing Feature 3 test users to remove.");
        return;
    }
    console.log(`Removing ${rows.length} existing Feature 3 user(s) and dependencies...`);

    await client.query(
        `DELETE FROM post_date_checkin WHERE reviewer_user_id IN (SELECT user_id FROM _f3pair)
            OR reviewed_user_id IN (SELECT user_id FROM _f3pair)`
    );

    const { rows: matchRows } = await client.query(
        `SELECT match_id FROM matches
         WHERE user1_id IN (SELECT user_id FROM _f3pair)
            OR user2_id IN (SELECT user_id FROM _f3pair)`
    );
    ...
    await client.query(
        `DELETE FROM survey_trigger WHERE user1_id IN (SELECT user_id FROM _f3pair)
            OR user2_id IN (SELECT user_id FROM _f3pair)`
    );
    await client.query(`DELETE FROM notifications WHERE user_id IN (SELECT user_id FROM _f3pair)`);
    await client.query(
        `DELETE FROM swipes WHERE swipe_user_id IN (SELECT user_id FROM _f3pair)
            OR swiped_user_id IN (SELECT user_id FROM _f3pair)`
    );
    await client.query(
        `DELETE FROM reports WHERE reported_user_id IN (SELECT user_id FROM _f3pair)
            OR reporter_user_id IN (SELECT user_id FROM _f3pair)`
    );
    await client.query(
        `DELETE FROM blocks WHERE blocker_user_id IN (SELECT user_id FROM _f3pair)
            OR blocked_user_id IN (SELECT user_id FROM _f3pair)`
    );
    await client.query(`DELETE FROM verification WHERE user_id IN (SELECT user_id FROM _f3pair)`);
    await client.query(
        `DELETE FROM moderation WHERE user_id IN (SELECT user_id FROM _f3pair)
            OR admin_id IN (SELECT user_id FROM _f3pair)`
    );
    await client.query(`DELETE FROM photo WHERE user_id IN (SELECT user_id FROM _f3pair)`);
    await client.query(`DELETE FROM trust_score WHERE user_id IN (SELECT user_id FROM _f3pair)`);
    await client.query(`DELETE FROM trust_score_history WHERE user_id IN (SELECT user_id FROM _f3pair)`).catch(() => {});
    await client.query(`DELETE FROM user_availability WHERE user_id IN (SELECT user_id FROM _f3pair)`).catch(() => {});

    await client.query(
        `DELETE FROM preference_genders WHERE preference_id IN (
            SELECT preference_id FROM preferences WHERE user_id IN (SELECT user_id FROM _f3pair)
        )`
    );
    await client.query(`DELETE FROM preferences WHERE user_id IN (SELECT user_id FROM _f3pair)`);
    await client.query(`DELETE FROM users WHERE user_id IN (SELECT user_id FROM _f3pair)`);
    console.log("Old Feature 3 users removed.");
}
...
async function clearMatchDataOnly(client, idA, idB) {
    ...
    if (mr.length === 0) {
        console.log("No match row between Feature 3 users — will recreate.");
        return;
    }
...
async function cmdReset() {
    ...
    try {
        ...
        if (rows.length < 2) {
            console.error(
                "Feature 3 test users not found. Run:  node scripts/feature3TestPair.js seed"
            );
            process.exit(1);
        }
        ...
        if (!idA || !idB) {
            console.error("Missing one of the Feature 3 emails. Run seed first.");
            process.exit(1);
        }
...
function printSuccess(matchId, idA, idB) {
    console.log("");
    console.log("── Feature 3 test pair ─────────────────────────────");
    ...
    console.log("");
    console.log("  Restart the backend server so in-memory conversation safety state clears.");
    console.log("────────────────────────────────────────────────────");
}
...
if (cmd === "seed") {
    ...
    console.error("Usage:  node scripts/feature3TestPair.js seed | reset");
    process.exit(1);
}
